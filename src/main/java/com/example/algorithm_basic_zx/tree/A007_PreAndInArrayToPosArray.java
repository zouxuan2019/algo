...
public class A007_PreAndInArrayToPosArray {

    public static int[] zouxuan(int[] pre, int[] in) {
        ...
        populatePost( pre, 0, N - 1, in, 0, N - 1, post, 0, N - 1, inPosition );
        return post;
    }

    public static void populatePost(int[] pre, int lPre, int rPre, int[] in, int lIn, int rIn, int[] post, int lPost, int rPost, HashMap<Integer, Integer> map) {
        if (lPre > rPre) {
            return;
        }
        int inHeadPos = map.get( pre[lPre] );
        post[rPost] = pre[lPre];
        populatePost( pre, lPre + 1, lPre + inHeadPos - lIn, in, lIn, inHeadPos - 1, post, lPost, lPost + inHeadPos - lIn - 1, map );
        populatePost( pre, lPre + inHeadPos - lIn + 1, rPre, in, inHeadPos + 1, rIn, post, lPost + inHeadPos - lIn, rPost - 1, map );
    }
