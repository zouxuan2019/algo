 ...
 *
 * */
public class A046_AOE_KillMonstor {
    // 纯暴力解法
    ...
    public static int minAoe2(int[] x, int[] hp, int range) {
        ...
        for (int i = 0; i < N; i++) {
            if (hp[i] > 0) {
                count += hp[i];
                removeBlood( hp, i, x, range );
            }
        }
        return count;
    }

    private static void removeBlood(int[] hp, int i, int[] x, int range) {
        int trigger = i;
        int N = x.length;
        ...
        int remove = hp[i];
        int right = trigger - 1;
        while (right < N && (x[right] - x[trigger - 1] <= range)) {
            right++;
        }
...
//			}
            ...
            if (ans2 != ans3) {
                ...
                System.out.println( "Hp: " + Arrays.toString( hp ) );
